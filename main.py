...
logger = logging.getLogger(name)  # ✅ Tuzatildi
...
async def b_kubok(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("⭐️ *3/10* — Titullar sonini yozing _(faqat raqam)_:", parse_mode="Markdown", reply_markup=back_kb())
    return B_TITUL
...
async def b_brawler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.text == "🔙 Orqaga":
        await update.message.reply_text("⭐️ *3/10* — Titulni qayta yozing:", parse_mode="Markdown", reply_markup=back_kb())
        return B_TITUL
    if not is_number(update.message.text):
...
async def b_gem(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("⚡️ *7/10* — Giperzaryad sonini yozing _(faqat raqam)_:", parse_mode="Markdown", reply_markup=back_kb())
    return B_GIPER
...
async def b_bpass(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.text == "🔙 Orqaga":
        await update.message.reply_text("⚡️ *7/10* — Giperzaryadni qayta yozing:", parse_mode="Markdown", reply_markup=back_kb())
        return B_GIPER
    if not is_number(update.message.text):
        await update.message.reply_text("❌ Faqat raqam kiriting!", parse_mode="Markdown")
        return B_BPASS
...
async def b_media(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elon = (
        ...
        f"📋 Nik: {d.get('nik', '-')}\n"
        f"🏆 Kubok: {d.get('kubok', '-')}\n"
        f"⭐️ Titul: {d.get('titul', '-')}\n"
        f"🥷 Brawler: {d.get('brawler', '-')}\n"
        f"🧝 Skinlar: {d.get('skin', '-')}\n"
        f"💎 Gem: {d.get('gem', '-')}\n"
        f"⚡️ Giperzaryad: {d.get('giper', '-')}\n"
